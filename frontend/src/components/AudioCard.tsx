...
const CreatorButton = ({ creator }: { creator: string }) => (
  <a
    ...
    className="bg-sky-600 rounded-md hover:bg-sky-700 text-white text-xs font-semibold px-3 py-1 transition"
  >
    Profile
  </a>
);
      ...
      <div className="flex items-center justify-center mt-3">
        ...
        {showPrivateIndicator && (
          <div className="flex items-center space-x-2 text-xs">
            ...
            <span
              className={`px-2 py-1 font-semibold rounded-md ${isPrivate ? 'bg-sky-600 text-white' : 'bg-emerald-500 text-white'
                }`}
            >
              {isPrivate ? 'Yes' : 'No'}
            </span>
